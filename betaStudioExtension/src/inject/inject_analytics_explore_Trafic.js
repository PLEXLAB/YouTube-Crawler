...
function getVideTabBody(){
	...
	for(var i = 0; i < numberOfCols - 1; i++){
		var colIndex	=	3 + i;
		var otherCol_v1	=	document.querySelectorAll("#row-container > div:nth-child(" + colIndex + ") > div.layout.horizontal.end-justified.style-scope.ytd-explore-table-row > div");
		var otherCol_v2	=	document.querySelectorAll("#row-container > div:nth-child(" + colIndex + ") > div.layout.horizontal.end-justified.style-scope.ytd-explore-table-row > div.value.debug-metric-value.style-scope.yta-explore-table-row");
		var otherColPerc= 	document.querySelectorAll("#row-container > div:nth-child(" + colIndex + ") > div.layout.horizontal.end-justified.style-scope.ytd-explore-table-row > div.formatted-percentage.style-scope.yta-explore-table-row");
		// console.log(otherCol_v1.length  + " otherCol_v1  ----------------->");
		// console.log(otherCol_v2.length  + " otherCol_v2  ----------------->");
		// console.log(otherColPerc.length + " otherColPerc ----------------->");
	...
	if(firstColFlag === true && otherColsFlag === true)
	{
		...
		setTimeout(function(){chrome.runtime.sendMessage({msg: "getAanalytics_explore_Geography", channelID: chID});}, config.getRandomTimeoutInterval());
		// setTimeout(function(){chrome.runtime.sendMessage("getAanalytics_explore_Geography");}, config.getRandomTimeoutInterval());
	}
}
