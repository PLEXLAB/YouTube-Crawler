...
chrome.runtime.onMessage.addListener(function(response, sender, sendResponse){
	if(isNaN(response))
	{
				...
				if( vID !== undefined && !(xBackup.includes(vID))){
					...
					chrome.tabs.create({windowId: windowId, url: 'https://studio.youtube.com/video/' + vID + '/analytics/./period-lifetime'},
						function(tab){
							...
							chrome.tabs.onUpdated.removeListener(completedListener);
								}
							}
							chrome.tabs.onUpdated.addListener(completedListener);
					});
				}
				else{
		...
		if (response.consent === "False" || response == "channelCrawled") { //Stop here
